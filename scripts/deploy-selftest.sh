#!/usr/bin/env bash
#
# One-command self-test deployment for a freshly uploaded MateClaw source tree.
#
# Usage:
#   bash scripts/deploy-selftest.sh
#
# The script intentionally does not require the operator to edit .env by hand:
# it creates .env with strong random secrets when missing, then runs
# `docker compose up -d --build`.
#
# Optional:
#   PUBLIC_URL=http://1.2.3.4:18080 bash scripts/deploy-selftest.sh
#   bash scripts/deploy-selftest.sh --public-url http://1.2.3.4:18080
#   bash scripts/deploy-selftest.sh --regen-env
...
usage() {
  cat <<'EOF'
MateClaw self-test deploy

Usage:
  bash scripts/deploy-selftest.sh [options]
...
rand_hex() {
  ...
  if command -v openssl >/dev/null 2>&1; then
    ...
    # POSIX fallback. dd reads an exact byte count, avoiding pipefail/SIGPIPE
    # issues from infinite /dev/urandom pipelines.
...
write_env() {
  ...
  cat > .env <<EOF
# Auto-generated by scripts/deploy-selftest.sh at ${generated_at}
# Safe for self-test deployment. For production hardening, review docs/zh/docker-deploy.md.
...
# Default to GHCR to avoid Docker Hub mirror 500/timeouts on some CN hosts.
SEARXNG_IMAGE=${SEARXNG_IMAGE:-ghcr.io/searxng/searxng:latest}
...
# Optional build acceleration, e.g. run:
...
wait_for_http() {
  ...
  echo "  docker compose logs -f mateclaw-server" >&2
  return 1
}
...
main() {
  ...
  if ! docker info >/dev/null 2>&1; then
    ...
    echo "Start Docker first, then rerun: bash scripts/deploy-selftest.sh" >&2
    exit 1
  fi
  ...
  echo
  echo "Self-test deployment started."
  echo "Open: ${public_url:-$local_url}"
  echo "Default login: admin / admin123"
  echo "After login, change the admin password and add your first model provider in Settings → Models."
  echo
  echo "Useful commands:"
  echo "  docker compose ps"
  echo "  docker compose logs -f mateclaw-server"
