...
去 [GitHub Releases](https://example.com/releases) 拿最新安装包。
...
- **后端起不来**——看日志文件（macOS：`~/Library/Application Support/HHAIOS/logs/mateclaw.log`；Windows：`%APPDATA%\HHAIOS\logs\mateclaw.log`）。桌面端后端使用动态端口，端口冲突会在日志里明确报出。
- **模型调用报错**——API Key 填错了，或者网络不通。回设置里检查，或者换一家试试。
- **界面白屏**——Ctrl/Cmd + Shift + R 强刷。Electron 的缓存比较顽固。
- **还是不行**——去 [GitHub Issues](https://example.com/support) 开一个 Issue，把 `app.log` 的尾巴贴上。我们真的会看。
...
- **公司集中部署 + Desktop 远程客户端**——服务端用 Docker 统一部署，员工安装 Remote/Lite Desktop 连接公司域名。看 [企业集中部署 + Desktop 远程客户端](./enterprise-remote-deploy)。
