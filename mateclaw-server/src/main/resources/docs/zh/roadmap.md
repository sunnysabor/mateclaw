...
| 砍掉的功能 | 为什么 | 什么时候才该做 |
|-----------|--------|--------------|
| **完整 RBAC 权限模型** | HHAIOS 是数字员工系统，不是企业管理平台。单团队不需要管理 100 种权限组合 | 当真正出现需要细粒度权限的多团队 SaaS 客户时 |
...
| **移动端原生 App** | 8 个 IM 渠道 + 桌面端 + Web 已经覆盖。你在手机上用钉钉 / 飞书 / Telegram 就在用 HHAIOS | 当 Web / IM 渠道有不可替代的移动专属能力时 |
