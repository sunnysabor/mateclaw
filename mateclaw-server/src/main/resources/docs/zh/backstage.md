...
数字员工在某一步冻住，是 HHAIOS 里少数不会自我修复的事情之一。流挂着、子 Agent 散到一地、SSE buffer 还活着但没人读。Backstage 是把这些东西全部摊开给你看、并且让你伸手干预的那块屏幕。
...
| 步骤 | 默认阈值 | 说明 |
|---|---|---|
| 短步骤（推理 chunk、状态更新） | 150 秒 | token 级活性 |
| 工具调用 | 600 秒 | 内置工具和 MCP / ACP 工具都算 |
| 整轮 | 1800 秒 | 端到端封顶 |

三者现在在 `AgentRuntimeAggregator` 里以静态阈值定义。reason 字符串典型形如 `tool_silent`、`idle_silent` 或 `hard_cap`，所以卡片不光告诉你"它死了"，还告诉你"为什么死的"。
