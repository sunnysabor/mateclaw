...
Web排队消息从V199起保存入队时已认证账户的内部ID，普通Web续跑同时携带当前会话工作区；V201还在入队时保存选定的受管Goal ID。若Goal在出队前终结，排队请求仍保留其身份，随后批准会拒绝旧选择。升级前没有选择快照的队列行，若会话有受管Goal历史，就保存为用户文字并要求重新发送。受管工具执行时仍重新校验账户、归属和当前要求。旧队列项不按用户名补造身份，不能用于受管JSON操作；需要用户重新发送已认证请求。持久Goal工作器消费输入时继续使用原有attempt owner校验，没有转换成免租约的账户路径。
