...
聊天是你真正干活的地方。HHAIOS 里其他所有东西——Agent、工具、记忆、Wiki、渠道——存在的理由，都是为了让**这个框里发生的事**足够好。
...
HHAIOS 的聊天 UI 在试着回答一个问题：**AI 刚刚告诉你的事情，该不该信？** 别处的默认答案是"看答案，自己猜"。HHAIOS 想做得更好。
...
在桌面端**拖一个文件夹**进来，Agent 拿到的是这个文件夹的绝对路径引用——它可以用文件读取或 shell 工具去遍历里面的文件。在 Web 端拖文件夹进来，HHAIOS 会递归展开、把每个文件单独上传。
...
::: tip 1.3.0 新增
当 Agent 配的主模型是纯文本模型（如 `deepseek-chat` / `kimi-k2`），上传图片不再"系统无法工作"，而是自动走旁路（sidecar）。详见 [issue #87](https://example.com/support/87)。
:::
...
一个会话是属于某个 Agent + 某个用户的消息序列。HHAIOS 用两张表存它们：
...
::: tip 1.4.0 新增
聊天顶栏的模型选择器现在把模型**绑定在会话上**，而不是全局开关。详见 [issue #150](https://example.com/support/150)。
:::
...
::: tip 1.4.0 新增
会话侧栏从一条单纯的历史列表，升级成了一个可操作的运营面板。详见 [issue #144](https://example.com/support/144)。
:::
...
每一个回合，HHAIOS 都会构造真正送进 LLM 的那个 prompt。大致是：
