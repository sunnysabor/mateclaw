export default {
  ...
  settings: {
    ...
    model: {
      ...
      claudeCodeOauthRevokeHint: '请在 Claude Code 客户端中退出登录。HHAIOS 不会修改 Claude Code 的本地凭据。',
  ...
  agents: {
    ...
    templates: {
      ...
      runtimeTitle: '选择运行方式',
      runtimeDesc: '本地员工使用 HHAIOS 技能和知识库；外部 Agent 直接运行已配置的 Hermes / Codex / OpenClaw。',
      templateDivider: '或从岗位模板开始',
      skip: '从空白开始',
      applied: '员工已上岗',
    },
    runtime: {
      localTitle: 'HHAIOS 本地员工',
      localDesc: '使用模型、技能、工具和知识库，适合日常业务岗位。',
      localBadge: '本地',
      externalDesc: '把 {name} 提升为员工运行时，聊天时直接调用外部 Agent。',
      externalBadge: '外部 Agent',
      externalDefaultDescription: '由 {name} 驱动的外部 ACP 员工',
    },
    teams: {
      kicker: '团队编排',
      title: '团队',
      desc: '把多个员工组成一个协作单元，由原生 HHAIOS 协调官统一拆解、委派和汇总。',
      create: '组建团队',
      newTitle: '组建团队',
      editTitle: '编辑团队',
      name: '团队名称',
      namePlaceholder: '例如：创业核心团队',
      description: '团队目标',
      descriptionPlaceholder: '这个团队负责什么、如何协作',
      noDescription: '暂无团队目标',
      coordinator: '团队协调官',
      selectCoordinator: '选择原生 HHAIOS 员工',
      coordinatorHint: '强制要求：协调官必须是 react / plan_execute 类型的原生 HHAIOS 员工；Hermes / Codex / OpenClaw 这类 ACP 员工只能作为成员。',
      nativeCoordinatorRequiredTitle: '必须有原生 HHAIOS 协调官',
      nativeCoordinatorRequiredDesc: '组建团队时需要一个本地员工负责调用 delegateToAgent / delegateParallel；ACP 员工可以加入团队，但不能担任协调官。',
      members: '团队成员',
      membersHint: '可选择 Hermes、Codex、OpenClaw 等 ACP 员工作为专家成员；协调官会自动加入成员列表。',
      selectedMembers: '已选 {count} 位',
      membersCount: '{count} 位成员',
      empty: '还没有团队。先点击“组建团队”，选择一个原生协调官，再加入 ACP 专家成员。',
      chatWithCoordinator: '进入团队对话',
      loadFailed: '加载团队失败',
      saveSuccess: '团队已保存，协调官身份卡已写入团队编排规则',
      saveFailed: '保存团队失败',
      deleteTitle: '删除团队',
      deleteConfirm: '确认删除团队「{name}」吗？员工不会被删除。',
      deleteSuccess: '团队已删除',
      deleteFailed: '删除团队失败',
      coordinatorRequired: '请选择团队协调官',
      coordinatorMustBeNative: '团队协调官必须是原生 HHAIOS 员工，不能选择 ACP 员工。',
    },
    search: '搜索员工...',
    tabs: {
      all: '全部',
    ...
    fields: {
      ...
      icon: '图标',
      type: '类型',
      acpEndpoint: '外部 Agent',
      acpEndpointHint: '第一版仅支持 Hermes、Codex、OpenClaw。请先在「设置 → ACP」启用对应连接。',
      acpRuntimeNote: 'ACP 员工直接运行外部 Agent，会话 30 分钟无活动后自动关闭；下次消息会根据历史记录恢复上下文。身份卡会保存并作为 HHAIOS 指令随每次消息注入；第一版不绑定 HHAIOS 技能、工具或知识库。',
      description: '描述',
      systemPrompt: '系统提示词',
      role: '岗位（Role）',
      ...
      taglinePreview: '卡片预览：',
      backstory: '身份卡 / 背景（Backstory）',
      backstoryHint: '员工身份卡放这里。用户看不见，但每一次对话都会生效；可写完整角色设定、经验、信念和做事原则。',
      advanced: '高级',
    ...
    placeholders: {
      ...
      role: '例：数据分析师',
      goal: '例：把数据变成可执行洞察',
      backstory: '粘贴身份卡，或写：在数据里待了十年，相信先问对问题再写 SQL...',
    ...
    wizard: {
      ...
      generate: '生成员工',
      generating: '正在生成…',
      runtimeKicker: '外部 Agent',
      runtimeTitle: '直接创建外部 Agent 员工',
      runtimeDesc: '已配置 Hermes / Codex / OpenClaw 时，可以跳过 AI 生成，直接把它们作为员工运行时。',
      tryLabel: '试试：',
  ...
  billing: {
    kicker: '账户余额',
    title: '充值中心',
    desc: '管理当前账号的钱包余额、充值套餐、订单和入账流水。',
    mockNoticeTitle: '当前是本地模拟支付',
    mockNoticeDesc: '这里不会跳转真实支付网关，也不会产生真实扣款；点击“模拟支付完成”只会在本地把订单置为已支付并写入钱包流水。',
    availableBalance: '可用余额',
    walletSubtitle: '后续可接入模型调用扣费或额度系统；当前仅记录充值余额。',
    pendingOrders: '待支付订单',
    recentCredits: '最近入账',
    packages: '充值套餐',
    packagesDesc: '选择套餐会先创建待支付订单，再由本地模拟支付完成入账。',
    noPackages: '暂无可用充值套餐',
    bonus: '赠 {amount}',
    packageFallbackDesc: '本地模拟充值套餐',
    creditTotal: '到账金额',
    createOrder: '创建订单',
    pendingOrdersDesc: 'MVP 阶段使用模拟支付完成订单，不接真实支付通道。',
    mockPay: '模拟支付完成',
    orders: '订单记录',
    ordersDesc: '展示最近的充值订单状态。',
    noOrders: '暂无订单记录',
    ledger: '钱包流水',
    ledgerDesc: '展示最近的余额变动。',
    noLedger: '暂无钱包流水',
    orderNo: '订单号',
    amount: '金额',
    status: '状态',
    createdAt: '创建时间',
    loadFailed: '加载充值信息失败',
    orderCreated: '充值订单已创建',
    orderCreateFailed: '创建订单失败',
    paySuccess: '模拟支付已完成，余额已入账',
    payFailed: '模拟支付失败',
  ...
  points: {
    kicker: '会员积分',
    title: '积分中心',
    desc: '查看 C 端用户积分余额、累计获取、累计消耗和积分流水。',
    rulesTitle: '积分模块已独立于充值余额',
    rulesDesc: '积分用于会员权益、活动奖励、兑换或未来额度体系；充值中心的钱包余额仍按金额独立记录。',
    ...
    totalSpent: '累计消耗',
    adminTitle: '管理员积分调整',
    adminDesc: '为用户手工增加或扣减积分，所有调整都会写入不可变流水。',
    targetUser: '目标用户',
    selectUser: '选择用户',
    adjustAmount: '调整积分',
    adjustAmountHint: '正数增加，负数扣减',
    adjustReason: '原因',
    adjustRemark: '备注',
    submitAdjust: '提交调整',
    ledger: '积分流水',
    ledgerDesc: '展示最近的积分变动记录。',
    noLedger: '暂无积分流水',
    reason: '原因',
    amount: '积分变动',
    balanceAfter: '变动后余额',
    ...
    adjustInvalid: '请选择用户，并填写非 0 的整数积分',
    adjustSuccess: '积分已调整',
    adjustFailed: '积分调整失败',
    levelMap: {
      normal: '普通会员',
  ...
  acp: {
    kicker: '外部 Agent',
    title: 'ACP 端点',
    desc: '通过 ACP/stdout-stdio 委派给 Hermes、Codex、OpenClaw。任务调用走 ACP，版本、安装和健康状态由 CLI 探测。',
    addEndpoint: '新增端点',
    builtin: '内置',
    test: '测试',
    testing: '测试中…',
    ...
    agents: {
      ...
      configured: '待启用',
      needsSetup: '需配置',
      useDetectedCommand: '使用检测路径',
      actions: {
        installCli: '安装 CLI，或把端点命令改成本机二进制绝对路径。',
        useDetectedCommand: '检测到的绝对路径可应用到端点命令。',
        fixAcp: '先修复 ACP 入口，再启用端点。',
        enableEndpoint: '连接测试通过后启用该端点。',
      },
    },
    env: {
      formMode: '编辑 JSON',
      ...
      hints: {
        ...
        dashscope: '阿里云 DashScope key — 跟 HHAIOS .env 里的 DASHSCOPE_API_KEY 同一个值',
  ...
  plugins: {
    ...
    noDescription: '暂无描述',
    emptyTitle: '暂无已安装插件',
    emptyHint: '将插件 JAR 文件放入已配置的插件目录后重启服务',
    loading: '加载插件中...',
    loadFailed: '加载插件失败',
    enabled: '{name} 已启用',
    disabled: '{name} 已禁用',
  ...
  triggers: {
    ...
    targetDifyWorkflow: '全局 Dify 工作流',
    targetDifyWorkflowEmpty: '尚未配置 Dify 工作流。',
    targetDifyAdminOnly: 'Dify 触发器使用全局 API Key，仅 global admin 可创建、修改、启停和删除。',
    patternTypeLabels: {
      cron: '定时任务',
  ...
  difyWorkflow: {
    title: 'Dify 工作流',
    desc: '配置一个全局 Dify Workflow App，通过手动运行或触发器启动，并在 HHAIOS 中保留完整运行记录。',
    warningTitle: '外部发送与持久化',
    warningBody: '运行时 inputs 会发送到 {baseUrl}，Dify 返回的完整 outputs、原始响应和错误信息会保存在 HHAIOS 运行记录中。',
    configTitle: '全局配置',
    configDesc: '第一版仅支持 Dify 官方云和一个全局 API Key。API Key 只写不读，保存后不会回显。',
    runTitle: '测试与手动运行',
    runDesc: '使用 blocking 模式调用 Dify，前端最多等待约 110 秒，后端读取超时为 95 秒。',
    runsTitle: '最近运行',
    runsDesc: 'Dify outputs 只保存在这里，不会自动投递到聊天或渠道消息。',
    enabled: '已启用',
    disabled: '已停用',
    testing: '测试中...',
    running: '运行中...',
    ...
    loadFailed: '加载 Dify 配置失败',
    runsLoadFailed: '加载 Dify 运行记录失败',
    saveSuccess: 'Dify 配置已保存',
    saveFailed: 'Dify 配置保存失败',
    testSubmitted: '测试运行已完成',
    runSubmitted: '手动运行已完成',
    runFailed: 'Dify 运行失败',
    invalidJson: '{label} 不是合法 JSON：{msg}',
    ...
    fields: {
      ...
      baseUrl: 'Base URL',
      apiKey: 'API Key',
      apiKeyPlaceholder: '粘贴 Dify Workflow App API Key',
      apiKeyConfigured: '已配置，留空保持不变',
      apiKeyHint: '仅在保存时写入；后端加密存储，前端不会读取或展示已有 Key。',
      enabled: '启用 Dify 工作流',
      enabledHint: '关闭后手动运行和触发器自动运行都会被后端拒绝。',
      inputSchemaJson: '输入说明 JSON',
      defaultInputsJson: '默认 inputs JSON',
      runInputsJson: '本次运行 inputs JSON',
    },
    detail: {
      triggeredBy: '来源',
      completedAt: '完成时间',
  ...
  login: {
    ...
    heroDesc: 'HHAIOS 的重点不是把更多能力堆进聊天框，而是把上下文、知识、执行和记忆收拢成一个可靠的工作面。',
  ...
  channels: {
    ...
    webchatHint: 'Web / API 接入既可把 HHAIOS 聊天挂件嵌入外部网站，也可作为纯后端 SSE 接口直接调用。配置 API Key 后，前端可引入 WebChat SDK；后端可直接 POST /api/v1/channels/webchat/stream，头带 X-MC-Key、体传 {message, visitorId, 可选 sessionId/agentId}。',
